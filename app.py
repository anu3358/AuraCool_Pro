...
st.markdown("""
    ...
    .section-header { color: #004085; font-size: 24px; font-weight: bold; border-bottom: 2px solid #004085; margin-bottom: 20px; }
    </style>
...
# --- HEADER ---
st.markdown('<div class="news-ticker">🌍 STATE COMMAND: Virtual Power Plant Synced | 💧 WATER ADVISORY: Evaporation tracking active</div>', unsafe_allow_html=True)
st.title("Punjab Climate Defense & Resource Command")

m1, m2, m3, m4 = st.columns(4)
m1.metric("State Avg. Temp", f"{round(df['temp'].mean(), 1)}°C")
m2.metric("Water Saved (Est.)", f"{round(total_water, 2)} ML")
...
with tab1:
    st.markdown('<div class="section-header">3D State-Wide Thermal View</div>', unsafe_allow_html=True)
...
with tab2:
    st.markdown('<div class="section-header">Resource Recovery: Solving Water Scarcity</div>', unsafe_allow_html=True)
...
with tab3:
    st.markdown('<div class="section-header">Sentinel Intelligence: Automated Auditing</div>', unsafe_allow_html=True)
    st.write("Using simulated Sentinel-3 Thermal Infrared data to detect policy violations.")
    anomalies = pd.DataFrame({
        "Zone": ["Industrial Area A", "Sector 12", "Truck Terminal"],
        "Heat Index": [48.2, 45.6, 49.1],
        "Compliance": ["❌ VIOLATION", "✅ OK", "⚠️ WARNING"]
    })
    st.table(anomalies)

with tab4:
    st.markdown('<div class="section-header">State Strategic Mandate</div>', unsafe_allow_html=True)
